...
const Fireflies = () => {
  ...
  const generateFireflies = () =>
    Array.from({ length: FIREFLY_CONFIG.TOTAL_FIREFLIES }, (_, i) => {
      ...
      const scale = isSpecial
        ? baseScale * FIREFLY_CONFIG.SPECIAL_SCALE_MULTIPLIER
        : baseScale;
      const color: "warm" | "cool" =
        Math.random() < FIREFLY_CONFIG.WARM_COLOR_PROBABILITY ? "warm" : "cool";
      const baseGlow =
        FIREFLY_CONFIG.BASE_GLOW_MIN +
        Math.random() *
          (FIREFLY_CONFIG.BASE_GLOW_MAX - FIREFLY_CONFIG.BASE_GLOW_MIN);
      const glow =
        baseGlow *
        (isSpecial ? FIREFLY_CONFIG.SPECIAL_GLOW_MULTIPLIER : 1) *
        (color === "warm" ? FIREFLY_CONFIG.WARM_GLOW_MULTIPLIER : 1);
      const hasScatter = Math.random() < FIREFLY_CONFIG.SCATTER_PROBABILITY;
      const scatterBlur = hasScatter
      ...
      return {
        ...
        initialY: inScreenY,
        delay: isSpecial
          ? FIREFLY_CONFIG.SPECIAL_DELAY_MIN + Math.random() * 18
          : Math.random() * 10,
        duration: isSpecial
          ? FIREFLY_CONFIG.SPECIAL_DURATION_MIN + Math.random() * 30
          : 15 + Math.random() * 25,
        blinkDuration: isSpecial
          ? 3 + Math.random() * 3
          : 2 + Math.random() * 4,
        scale,
        glow,
    ...
    <div className="absolute inset-0 pointer-events-none z-10">
      {fireflies.map((fly) => (
        <motion.div
          ...
          animate={{
            x: windAffectedIds.has(fly.id)
              ? fly.pathX.map((dx) => fly.initialX + dx + wind)
              : fly.pathX.map((dx) => fly.initialX + dx),
          ...
          transition={{
            x: {
              duration: fly.duration / 4,
              repeat: Infinity,
              ease: "easeOut",
            },
            y: { duration: fly.duration, repeat: Infinity, ease: "easeInOut" },
            opacity: {
              duration: fly.blinkDuration,
              repeat: Infinity,
              ease: "easeInOut",
            },
            scale: {
              duration: fly.duration / 2,
              repeat: Infinity,
              ease: "easeInOut",
            },
            delay: fly.delay,
